...
class LateArrivingDataGenerator(DataGenerator):
    ...
    def __init__(self, model_schema: dict, avro_schema:dict, delay_by_day: int = 0, delay_by_hour: int = 0):
...
class DataGeneratorFactory:
    ...
    @staticmethod
    def data_generator(generator_type: str,model_schema:dict,avro_schema: dict) -> DataGenerator:
        if generator_type == "streaming":
            return StreamingDataGenerator(model_schema=model_schema,avro_schema=avro_schema)
        elif generator_type == "late_arriving":
            return LateArrivingDataGenerator(model_schema=model_schema,avro_schema=avro_schema)
        else:
